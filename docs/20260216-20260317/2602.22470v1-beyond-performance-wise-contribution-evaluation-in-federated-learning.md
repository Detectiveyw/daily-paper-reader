...
## 摘要
联邦学习提供了一种隐私友好的协作学习框架，但其成功与任何合资项目一样，取决于参与者的贡献。现有的客户端评估方法主要集中在模型性能（如准确率或损失）上，这仅代表了机器学习模型整体效用的一个维度。相比之下，本研究探讨了客户端对模型可信度贡献这一关键但被忽视的问题，具体包括其可靠性（对噪声数据的容忍度）、鲁棒性（对抗攻击的抵御能力）和公平性（通过人口统计学平价衡量）。为了量化这些多维度的贡献，我们采用了 Shapley 值的最先进近似方法，这是一种规范的价值归因方法。我们的研究结果表明，没有单个客户端在所有维度上都表现优异，且各维度之间在很大程度上是相互独立的，这凸显了当前评估方案的一个关键缺陷：单一指标不足以进行全面评估和公平的奖励分配。
