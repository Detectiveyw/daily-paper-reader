...
score: 7.0
evidence: 大模型隐私推理
tldr: "本研究针对大语言模型在处理敏感数据时的隐私泄露及量子攻击风险，提出了一种基于完全同态加密（FHE）的Llama 3隐私保护推理方案。通过在Transformer架构中集成基于格密码的加密操作，并利用concrete-ml库优化推理流程，该方法在i9 CPU上实现了98%的生成准确率和每秒80个token的高效推理，为安全、抗量子的生成式AI应用提供了可行路径。"
selection_source: fresh_fetch
motivation: 生成式AI在处理医疗、金融等敏感数据时面临严重的数据泄露风险，且现有加密技术难以抵御未来的量子计算攻击。
method: 将基于格密码的完全同态加密（FHE）集成至Llama 3推理流水线，并利用concrete-ml库对Transformer架构的特定层进行加密改造。
result: "在i9 CPU上实现了高达98%的文本生成准确率，推理延迟仅为237ms，生成速度可达每秒80个token。"
conclusion: 实验证明了利用FHE实现抗量子攻击的LLM隐私推理在技术上是可行且高效的，成功平衡了安全性与推理性能。
---

## 摘要
生成式人工智能（GenAI）的应用及其与医疗、金融、交通和信息安全等数据驱动领域的交叉，显著提高了服务效率并降低了延迟。然而，这种协同作用也引发了关于大语言模型（LLM）安全性及其对公司和用户数据隐私潜在影响的严重担忧。许多在服务中集成 LLM 并具有一定指挥和控制权的科技公司，面临着由不安全的 LLM 流水线导致的数据泄露和秘密泄露风险，使其容易受到数据投毒、提示词注入和模型窃取等多种攻击。尽管已实施了多种安全技术（输入/输出清洗、去中心化学习、访问控制管理和加密）来降低此类风险，但量子计算攻击的风险依然迫在眉睫，预计这些攻击将破解现有的加密算法，从而获取密钥、加密的敏感数据并解密加密模型。在这项广泛的研究中，我们将基于后量子密码学（PQC）的格基同态加密（HE）主要功能集成到 LLM 的推理流水线中，以保护其部分层免受数据隐私攻击。我们修改了 LLAMA-3 模型的 Transformer 架构推理流水线，同时注入了 concrete-ml 库提供的核心同态加密操作。我们在 i9 CPU 上展示了极高的文本生成准确率（高达 98%）和合理的延迟（237 毫秒），生成速度可达每秒 80 个 token，这证明了运行受全同态加密（FHE）保护的 LLAMA-3 推理模型的可行性和有效性。文中还讨论了进一步的实验和分析，以解释模型的文本生成延迟和行为。
