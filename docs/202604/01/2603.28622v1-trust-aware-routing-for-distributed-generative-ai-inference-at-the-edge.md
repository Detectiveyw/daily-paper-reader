...
tldr: 针对边缘计算环境下分布式生成式AI推理面临的设备不可靠和信任缺失问题，本文提出G-TRAC框架。该框架将路由建模为风险受限最短路径问题，结合信任剪枝与Dijkstra算法实现低延迟路径选择，并采用混合信任架构维护全局声誉。实验证明，G-TRAC能有效隔离故障节点，显著提升推理任务的完成率和系统鲁棒性。
selection_source: fresh_fetch
motivation: 传统的尽力而为点对点路由无法应对边缘设备在分布式生成式AI推理中的性能波动、故障及潜在的恶意行为。
method: 提出G-TRAC框架，利用风险受限最短路径算法进行路径选择，并结合混合信任架构在稳定锚点与边缘节点间同步声誉。
result: 该方案在实际边缘规模下实现了亚毫秒级的路由延迟，并能有效隔离不可靠节点，在复杂网络环境下维持稳健执行。
conclusion: G-TRAC通过整合信任感知路由与系统级协议设计，为去中心化边缘环境下的生成式AI推理提供了可靠的协调保障。
---
