...
tags: ["query:fss-llm"]
score: 6.0
evidence: 基于分组密钥的分层安全聚合
tldr: "本研究探讨了具有组密钥的分层安全聚合问题，涉及一个服务器、多个中继和用户。研究设定了中继安全和服务器安全双重约束，即中继不获知任何信息，服务器仅获知输入总和。本文证明了当组大小 G=1 时问题不可行，并在 G>1 时完整刻画了最优速率区域。通过提出基于结构化预编码矩阵的线性编码方案，并建立匹配的逆定理，确定了用户、中继的最小传输速率以及最小组密钥速率。"
selection_source: fresh_fetch
motivation: 旨在解决分层网络架构下，如何利用组密钥在满足中继和服务器双重安全约束的同时实现高效的数据聚合。
method: 提出了一种基于结构化预编码矩阵的显式线性编码方案，利用大域上的通用矩阵设计来满足正确性和安全性。
result: 完整刻画了最优速率区域，确定了用户和中继的最小传输速率，并给出了满足安全约束的最小组密钥速率闭式解。
conclusion: 本文通过理论证明和方案构造，为分层安全聚合系统中的资源分配和密钥管理提供了最优性指导。
---

## 摘要
我们研究了具有分组密钥的分层安全聚合问题。该问题包含一个聚合服务器、$U$ 个中继和 $UV$ 个用户，其中每个中继服务 $V$ 个不相交的用户，且每 $G$ 个用户的子集共享一个独立的组密钥。该问题施加了两个安全要求：中继安全和服务器安全。具体而言，每个中继不得获知有关用户输入的任何信息，且服务器除了恢复的所有输入之和外，不得获知任何额外信息。我们首先证明当 $G = 1$ 时，该问题是不可行的。对于可行区间 $1 < G \le UV$，我们完整刻画了最优速率区域。特别地，我们证明了每个用户和每个中继对于每个输入符号必须至少传输一个符号。此外，我们将最小所需组密钥速率刻画为 $\max\left\{\frac{V}{\binom{UV}{G} - \binom{(U-1)V}{G}},\; \frac{U - 1}{\binom{UV}{G} - U \binom{V}{G}}\right\}$，其中这两项分别对应于中继安全和服务器安全所施加的约束。在可实现性方面，我们提出了一种基于结构化预编码矩阵的显式线性编码方案，并证明其满足正确性和安全性要求。该构造通过在有限大域上利用足够通用的矩阵设计，避免了基于置换的对称化。最后，我们建立了匹配的逆定理，从而刻画了最优速率区域。
