...
## 摘要
我们研究了两跳网络上多服务器安全聚合的基本极限，其中多个服务器分别连接到不相交的用户子集，并共同计算所有用户输入的总和。其目标是确保即使服务器与任意用户子集进行合谋，也无法推断出除所需聚合结果之外的任何关于指定输入子集的信息。现有工作主要集中在同质安全需求上，即保护所有输入免受不超过给定规模的合谋集合的影响。这种表述不足以涵盖更通用的场景，在这些场景中，不同的输入子集可能需要针对不同的合谋模式进行保护。在本文中，我们考虑了一个具有异构安全需求和任意用户合谋的通用模型。我们刻画了所有参数范围内的通信速率，并确定了大多数范围内安全聚合所需的最小密钥速率。特别地，我们在广泛的范围内建立了紧致的信息论下界和匹配的可达方案。对于剩余的范围，我们推导出了一个通用的下界，并给出了一个在有限差距内达到该下界的可达方案。我们的结果揭示了网络拓扑与异构安全约束之间的相互作用如何从根本上决定通信和密钥生成需求，并推广了现有的安全聚合结果。
