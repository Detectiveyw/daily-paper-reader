...
title_zh: 超越本地与外部之争：一种用于可信知识获取的博弈论框架
...
tldr: 针对云端大模型隐私泄露与本地模型能力不足的矛盾，本文提出GTKA博弈框架。该框架通过子查询生成器将敏感意图分解为低风险片段，利用对抗性攻击者评估泄露风险，并由本地模型整合外部知识。在生物医学和法律领域的实验证明，GTKA在有效保护隐私的同时，能显著提升知识获取的准确性。
selection_source: fresh_fetch
motivation: 旨在解决云端大模型在提供高质量知识获取服务时，可能导致用户敏感意图泄露的隐私安全难题。
method: 提出一种博弈论框架，通过对抗训练优化子查询生成策略，在最大化知识获取准确性的同时最小化意图的可重构性。
result: 在生物医学和法律领域的基准测试中，GTKA相比现有方法显著降低了意图泄露风险，并保持了高保真度的回答质量。
conclusion: GTKA为平衡大模型应用中的知识效用与隐私保护提供了一种有效的博弈论解决方案，实现了安全且高效的知识获取。
---

## 摘要
云端托管的大语言模型（LLMs）提供了无与伦比的推理能力和动态知识，但向这些外部服务提交原始查询存在泄露敏感用户意图的风险。相反，完全依赖受信任的本地模型虽然保护了隐私，但由于参数规模和知识有限，往往会损害回答质量。为了解决这一困境，我们提出了博弈论可信知识获取（GTKA），这是一个将知识效用与隐私之间的权衡建模为策略博弈的框架。GTKA 由三个组件组成：(i) 隐私感知子查询生成器，将敏感意图分解为泛化的、低风险的片段；(ii) 对抗性重构攻击者，尝试从这些片段中推断原始查询，并提供自适应泄露信号；(iii) 受信任的本地集成器，在安全边界内合成外部响应。通过以交替对抗的方式训练生成器和攻击者，GTKA 优化了子查询生成策略，以在最大化知识获取准确性的同时，最小化原始敏感意图的可重构性。为了验证我们的方法，我们在生物医学和法律领域构建了两个敏感领域基准。大量实验表明，与最先进的基准方法相比，GTKA 在保持高保真回答质量的同时，显著减少了意图泄露。
