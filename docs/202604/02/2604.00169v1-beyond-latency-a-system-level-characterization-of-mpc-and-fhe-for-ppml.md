...
title_zh: 超越延迟：针对隐私保护机器学习（PPML）的 MPC 与 FHE 系统级特性表征
authors: "Pengzhi Huang, Kiwan Maeng, G. Edward Suh"
date: 2026-03-31
pdf: "https://arxiv.org/pdf/2604.00169v1"
tags: ["query:fss-llm"]
score: 7.0
evidence: 隐私保护机器学习中多方计算的系统级特性分析
tldr: 本研究针对隐私保护机器学习（PPML）中安全多方计算（MPC）和全同态加密（FHE）评估标准碎片化的问题，提出了一种统一的系统级表征方法。通过在多种模型（CNN、Transformer）和网络环境（LAN、WAN）下对比评估三种主流方案，不仅关注推理延迟，还深入分析了能耗、货币成本及离线开销，为 PPML 的部署与优化提供了实证指导。
selection_source: fresh_fetch
motivation: 现有 PPML 评估通常局限于单一指标（如延迟）和特定场景，难以全面衡量不同技术在实际系统中的能效和成本表现。
method: 统一对比了基于算术/二进制共享转换、函数秘密共享的两种 MPC 变体以及 FHE 在不同模型规模、批处理大小和网络配置下的表现。
result: 评估涵盖了计算、通信、能耗和货币成本，揭示了不同隐私计算范式在离线与在线阶段的性能权衡。
conclusion: 为研究者和从业者提供了选择和加速 PPML 工作负载的系统级见解，并指出了未来硬件和网络趋势对技术选型的影响。
---

## 摘要
隐私保护已成为现代机器学习应用中日益受到关注的问题。隐私保护机器学习（PPML）吸引了越来越多的研究关注，安全多方计算（MPC）和全同态加密（FHE）等方法正被积极探索。然而，现有的对这些方法的评估通常是在狭窄、碎片化的设置下进行的，且仅关注特定的性能指标，例如特定批次大小的在线推理延迟。从现有报告中，很难对不同方法进行比较，特别是当考虑到能源/成本等其他指标或更广泛的系统设置（各种超参数、离线开销、未来的硬件/网络配置等）时。我们对三种流行方法——基于算术/二进制共享转换和函数秘密共享的两种 MPC 变体，以及 FHE——在多个 CNN 和 Transformer 模型上执行隐私保护推理的性能和成本进行了统一的特性表征。我们研究了一系列局域网（LAN）和广域网（WAN）环境、模型大小、批次大小以及输入序列长度。我们不仅评估了性能，还评估了在现实场景下部署的能耗和货币成本，并考虑了它们的离线和在线计算/通信开销。我们为选择、优化和部署这些隐私保护计算范式提供了经验性指导，并概述了不断发展的硬件和网络趋势将如何改变两种 MPC 方案与 FHE 之间的权衡。这项工作为寻求理解或加速 PPML 工作负载的研究人员和从业者提供了系统级的见解。
