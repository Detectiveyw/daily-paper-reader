...
tldr: 针对格密码私有信息检索（PIR）在服务器端面临的计算和内存瓶颈，本文提出了GPIR系统。GPIR利用GPU加速，通过引入阶段感知的混合执行模型、转置布局的GEMM设计以及细粒度流水线，解决了多客户端批处理带来的缓存失效和内存访问冲突问题。实验表明，GPIR在吞吐量上比现有最先进的PIRonGPU提升了高达305.7倍，显著增强了PIR的实用性。
selection_source: fresh_fetch
motivation: 多客户端批处理虽然提高了PIR吞吐量，但会导致工作集超出L2缓存，产生严重的内存访问冲突和性能瓶颈。
method: 提出阶段感知的混合执行模型以优化数据复用，并针对RowSel阶段设计了转置布局的GEMM和细粒度流水线。
result: GPIR在吞吐量上实现了显著提升，相比当前最先进的PIRonGPU系统，性能最高提升了305.7倍。
conclusion: 通过优化内核设计、数据布局和执行调度，GPIR成功克服了PIR在GPU上的内存瓶颈，使其在大规模实际应用中变得可行。
---
