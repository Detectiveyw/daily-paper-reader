...
title_zh: 代理见证：一种实用且可扩展的基于 TEE 的隐私保护审计方法
...
tldr: 针对专有数据审计中透明度与隐私权的冲突，本文提出 Agentic Witnessing 框架。该框架利用可信执行环境（TEE）隔离基于大语言模型（LLM）的审计代理，将验证从“证明执行”转向“证明推理”。通过布尔查询和加密转录，实现了在不泄露原始数据的情况下对非结构化定性属性（如代码逻辑）的隐私保护审计，为复杂数据的合规性检查提供了可扩展的解决方案。
selection_source: fresh_fetch
motivation: 传统的零知识证明难以处理非结构化数据的定性审计，且专有数据的隐私保护与验证需求之间存在天然矛盾。
method: 在 TEE 中部署 LLM 审计代理，通过模型上下文协议动态检查私有数据，并生成绑定硬件信任根的加密推理轨迹。
result: 成功对 21 篇计算机科学论文的私有代码库进行了自动化制品评估，验证了代码实现与论文描述的一致性。
conclusion: 该框架有效实现了定性验证与数据披露的解耦，为隐私保护下的合规监管提供了实用且可扩展的新途径。
---

## 摘要
对专有数据的语义属性进行审计会产生一种根本性的矛盾：验证需要透明的访问权限，而专有权则要求保密性。虽然零知识证明 (ZKP) 可以确保隐私，但它们通常局限于精确的代数约束，不适合验证定性的、非结构化的属性，例如代码库中的逻辑。我们提出了“代理见证”（Agentic Witnessing），这是一个将验证从“经证明的执行”转向“经证明的推理”的框架。该系统由三个代理组成：验证者（希望检查数据集属性）、证明者（拥有数据集）和审计者（负责检查数据集）。验证者被允许向审计者提出有限数量的简单二元真/假问题。通过将基于大语言模型 (LLM) 的审计者隔离在可信执行环境 (TEE) 中，该系统使验证者能够通过简单的布尔查询来查询证明者的私有数据，而无需暴露原始数据集。审计者使用模型上下文协议 (MCP) 动态检查目标数据集，生成是/否判定，并附带加密转录：一条将推理轨迹与原始数据集及 TEE 硬件信任根绑定的签名哈希链。我们通过自动化 21 篇在 GitHub 上发布了代码库的同行评审计算机科学论文的人造物评估（Artifact Evaluation）过程来演示该架构（例如：代码库是否实现了论文中描述的系统？）。我们在将源代码视为私有的情况下，验证了相应出版物中描述的这些代码库的五个高级属性。我们的结果表明，基于 TEE 的代理审计提供了一种隐私保护监管机制，有效地将定性验证与数据披露的需求解耦。
