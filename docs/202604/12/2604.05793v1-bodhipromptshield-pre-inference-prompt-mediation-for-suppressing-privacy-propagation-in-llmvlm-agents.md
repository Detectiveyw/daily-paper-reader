...
tldr: 针对大语言模型（LLM/VLM）智能体中隐私信息在检索、记忆和工具调用等环节跨阶段传播的问题，本文提出了 BodhiPromptShield 框架。该框架通过策略感知的预推理中介机制，利用占位符、语义抽象和安全符号映射对敏感信息进行脱敏，并延迟恢复至授权边界。实验表明，该方法在 CPPB 基准测试中有效抑制了隐私传播，在保持高准确率的同时优于传统脱敏方法。
selection_source: fresh_fetch
motivation: 现有脱敏方法无法解决隐私信息在智能体检索、记忆和工具调用等多个环节中的跨阶段传播风险。
method: 提出一种策略感知的预推理框架，通过敏感片段检测、语义抽象及符号映射处理隐私，并延迟恢复至授权边界。
result: "在 CPPB 基准测试中，该框架将跨阶段隐私传播率从 10.7% 降至 7.1%，且在隐私保护率和任务成功率上均优于通用脱敏方法。"
conclusion: BodhiPromptShield 通过显式的传播感知中介机制，为 LLM 智能体提供了一种比传统脱敏更有效的隐私保护方案。
---
