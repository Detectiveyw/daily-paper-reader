...
tags: ["query:fss-llm"]
score: 7.0
evidence: 使用多方安全计算技术的大模型隐私保护推理
tldr: LLM路由技术在平衡性能与成本的同时带来了隐私风险，而传统加密计算方案开销巨大。本文提出PPRoute框架，通过MPC友好型算子优化编码器推理、多步训练算法维持路由质量，并引入$O(1)$通信复杂度的Top-k算法加速模型搜索。实验表明，PPRoute在保持与明文相当性能的同时，比原生MPC实现提速约20倍，实现了高效且隐私保护的LLM路由。
selection_source: fresh_fetch
motivation: LLM路由层在动态选择模型服务时存在泄露用户隐私数据的风险，且现有加密计算方案在处理此类任务时效率极低。
method: 提出PPRoute框架，结合MPC友好算子、多步模型训练策略及高效的非排序Top-k搜索算法来优化加密域下的推理与搜索。
result: PPRoute在多个数据集上达到了与明文路由相当的准确率，且其推理速度比传统的MPC实现提升了约20倍。
conclusion: 该研究证明了通过针对性优化MPC协议与算法设计，可以在保障用户隐私的前提下实现高性能、低延迟的LLM路由服务。
---
