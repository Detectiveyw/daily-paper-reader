...
tags: ["query:fss-llm"]
score: 6.0
evidence: 分布式环境中的客户端可验证操作，以确保诚实执行
tldr: 针对低空无线网络（LAWN）中无人机与感知通信一体化（ISAC）设备的联邦学习场景，为满足“被遗忘权”并解决服务器不可信问题，本文提出了VerFU框架。该框架结合线性同态哈希（LHH）与承诺方案，实现了客户端可验证的联邦卸载，允许设备在不依赖原始数据的情况下验证服务器是否真实删除了其历史贡献，同时保证了高效的并行处理能力。
selection_source: fresh_fetch
motivation: 现有的联邦卸载方法通常假设服务器诚实，但在低空无线网络中，不可信服务器可能保留设备信息，从而导致隐私泄露风险。
method: 提出VerFU框架，利用线性同态哈希和承诺方案构建防篡改记录，使设备能通过验证去承诺参数来确保卸载结果的完整性。
result: 实验结果显示，该框架在完成卸载后能有效保持模型效用，同时维持了较低的通信和验证开销。
conclusion: VerFU为低空无线网络提供了一种安全、高效且可验证的联邦卸载方案，在保护隐私的同时增强了系统信任度。
---

## 摘要
在低空无线网络（LAWN）中，联邦学习（FL）能够在保持原始感知数据本地化的同时，实现无人机（UAV）与通感一体化（ISAC）设备之间的协作智能。由于“被遗忘权”的要求以及 ISAC 设备频繁进出无人机辅助服务器覆盖区域的高移动性，必须从已训练的模型中移除离网设备的影响。这种必要性促使在 LAWN 中采用联邦遗忘学习（FUL），以消除全局模型中历史设备的贡献。然而，现有的 FUL 方法隐含地假设无人机辅助服务器会诚实地执行遗忘操作。在缺乏客户端可验证保证的情况下，不可信的服务器可能会保留残留的设备信息，从而导致潜在的隐私泄露并破坏信任。为了解决这一问题，我们提出了 VerFU，这是一个专为 LAWN 设计的隐私保护且客户端可验证的联邦遗忘学习框架。它使 ISAC 设备能够在不依赖原始数据样本的情况下验证服务器端的遗忘操作。通过将线性同态哈希（LHH）与承诺方案相结合，VerFU 构建了历史更新的防篡改记录。ISAC 设备通过验证解承诺参数并利用 LHH 的线性可组合性来检查全局模型是否准确移除了其历史贡献，从而确保遗忘结果的完整性。此外，VerFU 能够高效处理来自多个 ISAC 设备的并行遗忘请求和验证。实验结果表明，我们的框架在保持低通信和验证开销的同时，能有效保留遗忘学习后的模型效用。
