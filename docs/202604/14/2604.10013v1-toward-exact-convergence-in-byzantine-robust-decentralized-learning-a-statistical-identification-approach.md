...
tags: ["query:fss-llm"]
score: 6.0
evidence: 去中心化学习中恶意节点的积极识别与剪枝
tldr: 针对去中心化学习中拜占庭攻击导致的收敛偏差问题，本文提出DRSGD-ByMI框架。该框架将防御策略从被动聚合转向主动识别，利用基于样本分割评分统计量的检测机制，在无需严格分布假设的情况下实现错误发现率控制并剔除恶意节点。理论证明该方法能恢复正常节点的连通性，使系统在存在攻击时仍能达到与标准去中心化随机一阶方法相同的最优收敛率，实现了精确收敛。
selection_source: fresh_fetch
motivation: 现有的鲁棒聚合方法因引入偏差而无法实现精确收敛，导致在拜占庭攻击下存在无法消除的稳态误差。
method: 提出一种基于“先检测后优化”管道的DRSGD-ByMI框架，通过样本分割评分统计量主动识别并剔除拜占庭节点。
result: 理论与实验表明，该方法能有效控制错误发现率并恢复网络连通性，实现与无攻击环境下相同的最优收敛速率。
conclusion: 该研究证明了通过统计识别手段可以克服拜占庭攻击带来的收敛瓶颈，为实现精确且鲁棒的去中心化学习提供了新路径。
---

## 摘要
为了防御去中心化学习中的拜占庭攻击，大多数现有方法依赖于鲁棒聚合规则来减轻恶意机器的影响。然而，这些策略本质上会引入偏差，导致收敛不精确且存在非消失的稳态误差。在本文中，我们通过引入带有拜占庭机器识别的去中心化重缩放随机梯度下降（DRSGD-ByMI）框架，提出了从被动聚合到主动识别的策略转变。我们方法的核心是一个基于识别的“先检测后优化”流水线，其中开发了一种无需 p 值的检测程序，用以从网络中准确剔除恶意节点。通过利用样本分割评分统计量，该识别机制在不需要限制性分布假设的情况下实现了错误发现率控制。我们从理论上证明，这种精确识别允许去中心化网络在正常节点之间恢复足够的连通性，从而使 DRSGD-ByMI 即使在存在拜占庭机器的情况下，也能达到与标准去中心化随机一阶方法相同的阶最优收敛率。数值实验验证了我们的理论结果，并证明了 DRSGD-ByMI 在去中心化鲁棒学习问题中的有效性。
