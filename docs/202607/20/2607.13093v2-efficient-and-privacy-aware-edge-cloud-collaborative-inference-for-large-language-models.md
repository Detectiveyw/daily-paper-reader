...
method: 设计了一种基于端点认证KV缓存的协作框架，将敏感及轻量级模块保留在本地，重负载计算交由云端，并利用加密和量化技术保护传输数据。
result: "实验结果显示，该框架相比基准切分推理减少了高达46.1%的每Token延迟和67.4%的下行数据负载。"
conclusion: 该研究证明了通过合理的边云任务划分与隐私保护机制，可以在异构边缘设备上实现高效且安全的大模型推理。
---
