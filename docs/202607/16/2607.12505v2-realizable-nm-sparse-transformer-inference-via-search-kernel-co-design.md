...
tags: ["query:fss-llm"]
score: 6.0
evidence: Transformer推理加速与硬件协同设计
tldr: M稀疏推理框架。硬件上，设计了MD-SpMM内核，通过微密集数据流和自适应并行提高GPU利用率；软件上，采用三阶段启发式搜索，在端到端延迟约束下优化各层稀疏配置。实验表明，该框架在保持精度的同时实现了超过2.2倍的加速，有效解决了稀疏性理论节省与实际加速不匹配的问题。
selection_source: fresh_fetch
motivation: M稀疏技术由于执行不规则性和硬件调度问题，导致理论计算节省难以转化为实际的GPU推理加速。
method: 提出软硬件协同设计，包括支持Tensor Core对齐的MD-SpMM稀疏内核以及基于延迟预算的三阶段启发式层级稀疏搜索。
result: 在多个ViT和Swin模型上实现了超过2.2倍的推理加速，并在相同延迟约束下保持了优异的模型精度。
conclusion: 通过内核执行与稀疏配置的深度协同优化，能够显著提升视觉Transformer在现代硬件上的实际部署效率。
---

## 摘要
视觉 Transformer (ViT) 取得了极高的准确率，但推理延迟也较高。半结构化 N:M 稀疏可以降低计算成本，但其理论上的节省往往无法在现代 GPU 上转化为成比例的端到端加速。这种不匹配的原因在于，部署延迟不仅取决于计算量的减少，还取决于稀疏情况下的执行规整性和硬件调度。因此，要实现实际加速，需要对稀疏执行和稀疏配置进行协同设计。为此，我们提出了一个用于 N:M 稀疏 ViT 推理的软硬件协同设计框架。在硬件方面，我们设计了 MD-SpMM，这是一种 N:M 稀疏 CUDA 算子，它将稀疏 GEMM 重新组织为微密集、Tensor Core 对齐的数据流，并利用推理感知的自适应并行性来维持利用率。在软件方面，我们在明确的端到端延迟预算下，使用带有约束松弛的三阶段启发式搜索进行逐层稀疏度搜索，以避免过早收敛并实现部署感知的稀疏度分配。在多个 ViT/Swin 模型和 GPU 平台上的实验表明，该框架在保持相当准确率的同时实现了超过 2.2 倍的延迟加速，并在相同的延迟约束下提供了更高的准确率。源代码已在 https://github.com/liuganhuo/realizable-nm-sparse-transformer 公开。
