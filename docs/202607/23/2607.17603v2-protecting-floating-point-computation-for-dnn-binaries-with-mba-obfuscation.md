...
title_zh: 利用 MBA 混淆保护 DNN 二进制文件的浮点运算
...
tags: ["query:fss-llm"]
score: 6.0
evidence: 保护深度神经网络二进制文件的浮点运算
tldr: 针对深度神经网络（DNN）二进制文件易受逆向工程攻击导致模型泄露的问题，本文提出了一种名为FLOB的混淆框架。FLOB利用混合布尔算术（MBA）变换保护浮点运算，通过将浮点值提升至高精度二进制扩展空间进行变换，确保在不引入额外舍入误差的前提下实现语义等效。实验表明，FLOB能显著降低算子恢复率，在保护模型资产的同时兼顾计算准确性与性能。
selection_source: fresh_fetch
motivation: 传统的代码混淆技术主要针对整数运算和复杂控制流，难以有效保护以浮点运算为主且结构简单的DNN二进制模型。
method: 提出FLOB框架，将浮点数提升至高精度二进制扩展空间进行MBA变换，并在计算完成后投影回原精度以保证数值一致性。
result: "实验显示FLOB将算子恢复率平均降低至4.51%，相比现有方法降低了32.82个百分点，且未引入额外的数值误差。"
conclusion: FLOB为DNN二进制文件提供了一种高效且保真的浮点运算混淆方案，在安全性与系统性能之间实现了良好的平衡。
---

## 摘要
深度神经网络 (DNN) 已成为现代计算系统的基础组件，具有广泛的应用，如计算机视觉、边缘智能等。为了实现低延迟和数据隐私，DNN 模型越来越多地被编译成可执行文件并部署在本地设备上。然而，这使模型面临被窃取的风险，使攻击者能够通过逆向工程技术恢复专有资产。虽然代码混淆自然地成为了保护可执行文件免受逆向工程影响的手段，但现有的方案主要针对传统程序设计，侧重于复杂的控制流结构和基于整数的操作。它们从根本上不适用于 DNN 二进制文件，因为后者表现出相对简单的代码结构，且严重依赖浮点运算。在本文中，我们提出了 FLOB，这是一个利用混合布尔算术 (MBA) 变换保护 DNN 二进制文件中浮点运算的混淆框架。我们的方法将浮点值提升到更高精度的二进制扩展空间，并在表示层执行 MBA 变换。计算完全在提升后的空间中进行，最终结果再投影回目标精度，从而在原始浮点格式下产生语义等效的输出，且不引入额外的舍入误差。实验结果表明，FLOB 在对抗现有的逆向工程分析器和反混淆器方面优于最先进的混淆方案，同时在不引入额外数值误差的情况下保持了计算正确性，并允许在保护和性能之间进行灵活权衡。具体而言，FLOB 将算子恢复率平均降低至 4.51%，与现有方法相比降低了 32.82 个百分点。
