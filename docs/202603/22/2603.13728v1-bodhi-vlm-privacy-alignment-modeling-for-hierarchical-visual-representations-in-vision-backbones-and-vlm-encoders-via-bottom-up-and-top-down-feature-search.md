...
## 摘要
保护隐私的学习系统通常在层级视觉表示中注入噪声；其核心挑战在于如何以一种可解释且适用于视觉骨干网络和视觉语言模型（VLM）的方式，对这些扰动与声明的隐私预算之间的对齐进行建模。我们提出了 Bodhi VLM，这是一个针对层级神经表示的隐私对齐建模框架：它 (1) 通过基于 NCP 和 MDAV 的聚类将敏感概念与逐层分组联系起来；(2) 在多尺度表示（如特征金字塔或视觉编码器层）上利用自底向上（BUA）和自顶向下（TDA）策略定位敏感特征区域；(3) 使用期望最大化隐私评估（EMPA）模块，通过将拟合的敏感特征分布与评估者指定的参考分布（如尺度为 $c/ε$ 的拉普拉斯或高斯分布）进行对比，生成可解释的预算对齐信号。该输出是相对于参考值的，并非正式的差分隐私估计器。我们在层级特征结构上形式化了 BUA/TDA，并在目标检测器（YOLO、PPDPTS、DETR）以及 VLM 的视觉编码器（CLIP、LLaVA、BLIP）上验证了该框架。BUA 和 TDA 表现出相当的偏差趋势；在所报告的实验设置下，EMPA 提供了稳定的对齐信号。我们将其与通用差异基准（卡方、K-L、MMD）以及任务相关基准（MomentReg、NoiseMLE、Wass-1）进行了对比。结果以多个随机种子的均值±标准差形式报告，置信区间详见补充材料。本研究为隐私对齐的层级表示提供了一个可学习、可解释的建模视角，而非仅限于事后审计。源代码：https://github.com/mabo1215/bodhi-vlm.git
