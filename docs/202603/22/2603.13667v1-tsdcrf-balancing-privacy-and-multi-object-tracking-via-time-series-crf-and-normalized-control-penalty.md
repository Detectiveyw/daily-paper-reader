...
tags: ["query:ppml"]
score: 7.0
evidence: 追踪中敏感身份信息的差分隐私保护
tldr: 针对多目标跟踪中隐私保护与跟踪性能的冲突，本文提出TSDCRF框架。该框架通过差分隐私高斯噪声保护敏感区域，利用归一化控制惩罚（NCP）稳定关联，并结合时间序列动态条件随机场（DCRF）修正轨迹偏差。实验证明，该方法在MOT16/17等数据集上实现了优于现有方法的隐私与效用平衡，有效减少了ID切换并增强了鲁棒性。
selection_source: fresh_fetch
motivation: 传统的隐私保护噪声会破坏多目标跟踪的跨帧关联，导致频繁的ID切换和目标丢失。
method: 提出一种包含差分隐私噪声注入、归一化控制惩罚（NCP）和时间序列动态条件随机场（DCRF）的插件式优化框架。
result: 在MOT16、MOT17等数据集上，该方法在隐私保护强度、跟踪精度和抗轨迹劫持能力方面均优于现有技术。
conclusion: TSDCRF通过结合时序一致性约束和噪声前处理，成功解决了多目标跟踪中隐私保护与数据效用之间的权衡难题。
---
