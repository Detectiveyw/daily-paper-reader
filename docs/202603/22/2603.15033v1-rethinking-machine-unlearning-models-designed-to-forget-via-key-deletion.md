...
motivation: 传统机器遗忘方法多为事后处理，依赖完整训练数据且需频繁更新参数，难以满足实际部署中快速、低成本的遗忘需求。
method: 提出MUNKEY模型，利用记忆增强型Transformer将实例记忆与权重解耦，通过删除特定键值实现无需更新权重的零样本遗忘。
result: 在自然图像、细粒度识别和医疗数据集上的实验表明，MUNKEY在保持预测性能的同时，遗忘效果优于所有主流的事后处理基准。
conclusion: “设计即遗忘”范式证明了通过架构创新可使模型具备原生遗忘能力，从而实现更快速、更实用的机器遗忘部署。
---
