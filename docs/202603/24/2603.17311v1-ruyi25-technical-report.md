...
tldr: 本研究推出了多模态家族模型Ruyi2.5，基于AI Flow框架延续了“一次训练，多处部署”的范式，通过共享骨干架构实现不同规模模型间的语义一致性。针对隐私保护需求，开发了Ruyi2.5-Camera系统，利用边缘端不可逆特征映射进行脱敏处理。此外，提出BPPO算法优化强化学习微调，使训练速度提升2-3倍。实验显示其在通用及隐私监控任务中均表现卓越。
selection_source: fresh_fetch
motivation: 旨在解决多模态模型在跨规模部署时的语义一致性挑战，并提升监控场景中的隐私保护能力与强化学习的训练效率。
method: 采用共享骨干架构协同训练多尺度模型，并结合信息瓶颈引导的特征映射技术及二元前缀策略优化（BPPO）算法。
result: Ruyi2.5在通用基准上对标Qwen3-VL，且在隐私受限的监控任务中表现更优，同时BPPO显著提升了训练速度。
conclusion: Ruyi2.5通过创新的统一架构和高效优化算法，为构建高性能、隐私安全且易于部署的多模态模型家族提供了有效方案。
---

## 摘要
我们提出了 Ruyi2.5，这是一个基于 AI Flow 框架构建的多模态家族模型。Ruyi2.5 将 Ruyi2 的“一次训练，多次部署”范式扩展到多模态领域，构建了一个共享主干网络架构，在单一统一流水线中协同训练不同规模的模型，确保了所有部署层级之间的语义一致性。基于 Ruyi2.5，我们开发了 Ruyi2.5-Camera 模型作为一种隐私保护摄像头服务系统，该系统将 Ruyi2.5-Camera 实例化为一个两阶段识别流水线：边缘端模型利用信息瓶颈引导的不可逆特征映射在源头对原始帧进行去标识化处理，而云端模型则进行深层行为推理。为了加速强化学习微调，我们进一步提出了二值前缀策略优化（BPPO），该方法通过二值响应选择减少样本冗余，并将梯度更新集中在响应前缀上，相比 GRPO 实现了 2 到 3 倍的训练加速。实验表明，Ruyi2.5 在通用多模态基准测试中与 Qwen3-VL 表现相当，而 Ruyi2.5-Camera 在隐私受限的监控任务中显著优于 Qwen3-VL。
