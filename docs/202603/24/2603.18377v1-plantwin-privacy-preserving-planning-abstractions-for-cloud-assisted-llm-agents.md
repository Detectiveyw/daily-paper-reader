...
## 摘要
云端托管的大语言模型（LLM）已成为智能体系统（agentic systems）中事实上的规划器，负责协调工具并指导本地环境中的执行。然而，在许多部署场景中，被规划的环境是私有的，包含源代码、文件、凭据和元数据，这些信息不能暴露给云端。现有解决方案解决了相关问题，如执行隔离、访问控制或机密推理，但它们并未控制云端规划器在规划过程中观察到的内容：在许可范围内，原始环境状态仍然会被暴露。我们提出了 PlanTwin，这是一种用于云辅助规划的隐私保护架构，无需暴露原始本地上下文。其核心思想是将真实环境投影到一个“面向规划的数字孪生”（planning-oriented digital twin）中：这是一个受模式约束且经过脱敏处理的抽象图，它保留了与规划相关的结构，同时移除了可重构的细节。云端规划器仅通过受限的能力接口在该净化后的孪生体上运行，而本地网关则负责执行安全策略和累积披露预算。我们进一步将隐私与效用之间的权衡形式化为能力粒度问题，利用 $(k,δ)$-匿名性和 $ε$-不可链接性定义了架构隐私目标，并通过多轮披露控制来缓解组合泄露。我们将 PlanTwin 实现为本地智能体与云端规划器之间的中间件，并在 10 个领域的 60 个智能体任务上使用 4 种云端规划器进行了评估。PlanTwin 实现了完全的敏感项非披露（SND = 1.0），同时保持了接近全上下文系统的规划质量：四分之三的规划器实现了 PQS > 0.79，且整个流水线的效用损失低于 2.2%。
