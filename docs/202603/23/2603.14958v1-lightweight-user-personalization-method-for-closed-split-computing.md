...
tldr: 针对封闭式拆分计算中因数据偏移、通信不稳定和隐私扰动导致的性能下降问题，本文提出了SALT（拆分自适应轻量级微调）框架。该框架通过在客户端引入紧凑的适配器来优化冻结头部网络的中间表示，无需修改原始模型架构或增加通信开销。实验证明，SALT在提升用户个性化准确率的同时，显著降低了训练延迟，并在高丢包率和噪声环境下表现出极强的鲁棒性，为边缘协作推理提供了高效的适配方案。
selection_source: fresh_fetch
motivation: 旨在解决封闭拆分计算环境下，由于模型架构不可访问及用户数据分布偏移、网络不稳定等因素导致的推理性能退化问题。
method: 引入一种名为SALT的轻量级适配框架，通过在客户端微调中间表示来适配冻结的头部和服务器端网络，而无需改变原始模型参数。
result: "实验显示SALT将CIFAR-10的个性化准确率提升至93.8%并降低了60%以上的训练延迟，同时在极端丢包和噪声注入下保持了高准确率。"
conclusion: SALT证明了通过轻量级客户端适配器可以有效实现拆分计算系统的个性化与鲁棒性，是处理实际部署挑战的实用方案。
---
