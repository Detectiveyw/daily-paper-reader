...
title_zh: 以遗忘进行攻击：图神经网络中由机器遗忘诱导的对抗攻击
...
## 摘要
图神经网络 (GNNs) 广泛用于社交网络、推荐系统和金融平台等领域的图结构数据学习。为了遵守 GDPR、CCPA 和 PIPEDA 等隐私法规，近似图遗忘 (approximate graph unlearning) 已成为可信图学习中日益重要的组成部分，其旨在无需完全重新训练的情况下，从已训练模型中消除特定数据点的影响。然而，近似遗忘通常会导致细微的性能下降，这可能会产生负面的、非预期的副作用。在这项工作中，我们展示了这种性能下降可以被放大为对抗攻击。我们提出了“遗忘破坏攻击” (unlearning corruption attacks) 的概念，即攻击者向训练图中注入精心选择的节点，随后请求删除这些节点。由于删除请求是法律强制要求的且无法被拒绝，这种攻击面既不可避免又具有隐蔽性：模型在训练期间表现正常，但仅在执行遗忘操作后准确率才会崩溃。在技术上，我们将此攻击建模为一个双层优化问题：为了克服黑盒遗忘和标签稀缺的挑战，我们通过基于梯度的更新来近似遗忘过程，并利用代理模型生成伪标签进行优化。在多个基准测试和遗忘算法上的广泛实验表明，少量精心设计的遗忘请求即可诱发显著的准确率下降，这引发了对现实监管需求下 GNN 遗忘鲁棒性的紧迫关注。源代码将在论文被接收后发布。
