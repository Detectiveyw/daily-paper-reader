...
title_zh: 变废为宝：跨机构联邦学习中用于可验证聚合的瞬态内在证明
...
tldr: 针对跨机构联邦学习中安全聚合无法保证聚合完整性的问题，本文提出一种轻量级的“内在证明”架构。该方法巧妙地将后门注入技术转化为验证信号嵌入模型参数中，利用灾难性遗忘特性使信号在验证后自然衰减，从而在不依赖重型密码学工具的情况下，实现了高效、可验证且不损模型性能的聚合校验，相比传统方案提速超千倍。
selection_source: fresh_fetch
motivation: 现有的可验证聚合方案依赖高开销的密码学技术，难以扩展至大规模模型，且无法有效防止恶意服务器篡改聚合结果。
method: 提出将后门注入作为内在证明信号嵌入模型，并结合随机化审计框架与灾难性遗忘特性，实现信号的即时验证与自动消除。
result: 在多个数据集上的实验表明，该方法能高概率检测恶意行为，且在ResNet-18模型上的处理速度比传统密码学方案快1000倍以上。
conclusion: 本研究证明了利用模型自身特性（如后门和遗忘）可以构建极低开销的完整性验证机制，为大规模联邦学习的安全聚合提供了新思路。
---

## 摘要
虽然安全聚合（SA）保护了跨机构联邦学习中更新的机密性，但它无法保证聚合的完整性，使得恶意服务器能够静默地忽略或篡改更新。现有的可验证聚合方案依赖于重量级密码学技术（如零知识证明 ZKP、同态加密 HE），导致计算开销随模型规模增加而扩展性较差。在本文中，我们提出了一种轻量级架构，将外部密码学证明转变为“内在证明”（Intrinsic Proofs）。我们重新利用后门注入技术，将验证信号直接嵌入到模型参数中。通过利用灾难性遗忘（Catastrophic Forgetting），这些信号对于即时验证是稳健的，但又是瞬态的，会自然衰减以保留最终模型的效用。我们设计了一个与 SA 兼容的随机化单验证者审计框架，在无需可信第三方的情况下确保客户端匿名性并防止信号冲突。在 SVHN、CIFAR-10 和 CIFAR-100 上的实验表明，该方法对恶意服务器具有较高的检测概率。值得注意的是，与密码学基准相比，我们的方法在 ResNet-18 上实现了超过 1000 倍的加速，能够有效扩展到大型模型。
