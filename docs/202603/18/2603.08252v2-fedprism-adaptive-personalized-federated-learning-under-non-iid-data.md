...
tldr: 针对联邦学习中非独立同分布（Non-IID）数据导致的性能下降问题，本文提出FedPrism框架。该框架通过棱镜分解方法将模型分为全局、共享组和私有三部分，实现自动聚类与动态调整；同时采用双流设计，根据置信度在通用模型与本地专家模型间路由预测。实验证明，FedPrism在高度异构环境下显著优于现有基准，有效平衡了全局泛化与自适应个性化。
selection_source: carryover_cache
motivation: 传统的联邦学习全局聚合策略难以应对现实中极端的统计异构性，导致本地个性化效果不佳。
method: 提出棱镜分解方法构建三层模型结构，并结合双流设计根据置信度动态路由预测。
result: 在多种非独立同分布数据分区实验中，FedPrism的准确率显著超过了静态聚合和硬聚类基准。
conclusion: FedPrism为异构环境下的联邦学习提供了一种鲁棒且灵活的方案，实现了泛化知识与个性化需求的有效平衡。
---

## 摘要
联邦学习（FL）在具有中度到极端统计异构性（非独立同分布/non-IID 客户端数据）的实际部署中面临显著的性能下降。虽然全局聚合策略促进了广泛的泛化能力，但它们往往无法捕捉本地数据分布的多样性，导致个性化效果欠佳。我们通过 FedPrism 框架解决了这一问题，该框架采用了两种主要策略。首先，它使用一种棱镜分解（Prism Decomposition）方法，将每个客户端的模型构建为三个部分：全局基础部分、相似客户端的共享组部分以及针对唯一本地数据的私有部分。这使得系统能够自动将相似用户分组，并在其数据发生变化时进行自适应调整。其次，我们引入了双流（Dual-Stream）设计，让通用模型与本地专家模型并行运行。系统根据专家模型的置信度，在通用模型和本地专家模型之间路由预测。通过对非独立同分布数据分区的系统实验，我们证明了 FedPrism 优于静态聚合和硬聚类基准，在高异构性下实现了显著的准确率提升。这些结果表明，FedPrism 是异构环境下联邦学习的一种稳健且灵活的解决方案，有效地平衡了可泛化知识与自适应个性化。
