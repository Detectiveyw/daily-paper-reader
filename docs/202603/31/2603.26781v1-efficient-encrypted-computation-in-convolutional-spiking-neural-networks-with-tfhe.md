...
## 摘要
随着人工智能技术的飞速发展，数据隐私问题日益受到关注，从而引发了关于加密计算机器学习的前沿研究。全同态加密（FHE）是隐私保护计算的一项关键技术，但由于其在离散整数上运行且仅支持加法和乘法，在处理连续非多项式函数时面临挑战。脉冲神经网络（SNNs）使用离散脉冲信号，天然地与 FHE 的特性互补。在本文中，我们介绍了 FHE-DiCSNN，这是一个基于 TFHE 方案构建的框架，利用 SNN 的离散特性进行安全高效的计算。通过利用自举（bootstrapping）技术，我们成功地在密文上实现了漏失累积发放（LIF）神经元模型，从而允许构建任意深度的 SNN。我们的框架可适配其他脉冲神经元模型，为 SNN 的同态评估提供了一种新方法。此外，我们集成了受 CNN 启发的卷积方法，以提高准确性并减少与随机编码相关的仿真时间。并行计算技术进一步加速了自举操作。在 MNIST 和 FashionMNIST 数据集上的实验结果验证了 FHE-DiCSNN 的有效性，与明文相比，准确率损失分别小于 3%，且每次预测的计算时间在 1 秒以内。我们还将该模型应用于实际的医学图像分类问题，并分析了参数优化与选择。
