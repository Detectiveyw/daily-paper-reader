...
tldr: 本研究针对联邦推荐系统中用户行为动态演变导致的时间遗忘和协作个性化不足问题，提出了FCUCR框架。该框架通过时间感知自蒸馏策略在本地更新中隐式保留历史偏好，并设计了用户间原型传输机制，在保护隐私的前提下利用相似用户的知识增强个体表征。实验证明，FCUCR在四个公开数据集上均表现优异，有效实现了隐私保护下的长期个性化推荐。
selection_source: fresh_fetch
motivation: 现有的联邦推荐系统往往忽略了用户行为的动态性，导致模型在适应新偏好时容易遗忘历史信息且难以在异构数据下实现有效的协作个性化。
method: 提出了FCUCR框架，结合时间感知自蒸馏策略以缓解时间遗忘，并利用用户间原型传输机制在保护隐私的同时实现跨用户的知识共享。
result: 在四个公共基准数据集上的实验结果表明，FCUCR在推荐性能上显著优于现有方法，并展现出良好的兼容性和实际应用潜力。
conclusion: FCUCR成功整合了联邦学习与持续学习，为在隐私受限环境下处理用户演化偏好和提升协作推荐质量提供了有效的解决方案。
---
