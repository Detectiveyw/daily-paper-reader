...
tldr: 针对大型视觉语言模型（LVLM）在处理敏感信息时存在的隐私泄露风险，本文提出了一种名为 Neural Gate 的神经元级模型编辑方法。该方法通过学习特征向量精准定位与隐私概念相关的神经元，并据此引导参数更新，从而提高模型对隐私查询的拒绝率。实验证明，该方法不仅能有效应对未见过的隐私请求，还能在增强安全性的同时保持模型在标准任务上的原有性能，为 LVLM 的安全部署提供了新思路。
selection_source: fresh_fetch
motivation: 现有的 LVLM 隐私保护方法在处理未知隐私查询时泛化性不足，且容易损害模型在常规任务上的表现。
method: 提出 Neural Gate 方法，通过学习特征向量识别并定位模型中与隐私相关的神经元，实现精准的梯度门控参数编辑。
result: 在 MiniGPT 和 LLaVA 上的实验表明，该方法显著提升了隐私保护能力和泛化性，且未对模型原有功能造成破坏。
conclusion: Neural Gate 证明了通过神经元级精准编辑可以有效平衡 LVLM 的隐私安全与任务效用。
---
