...
tldr: 针对物联网（IoT）环境中不断演变的威胁和概念漂移问题，本研究提出了一种增量联邦学习框架用于入侵检测。通过在联邦学习设置下采用LSTM模型，并结合数据和模型驱动的增量学习策略，解决了模型在更新过程中对旧攻击模式的灾难性遗忘。实验基于CICIoMT2024数据集，验证了不同策略在检测性能与计算延迟之间的权衡，为构建资源受限下的弹性IDS提供了指导。
selection_source: fresh_fetch
motivation: 传统的入侵检测系统在面对物联网中不断变化的攻击手段（概念漂移）时，性能会随时间下降且难以兼顾隐私与资源限制。
method: 在联邦学习框架中部署LSTM模型，并对比分析了累积增量学习、代表性学习及基于保留的方法在应对非平稳数据流时的表现。
result: 实验表明累积增量学习和代表性学习在漂移条件下表现最稳定，而基于保留的方法在准确率和延迟之间达到了良好的平衡。
conclusion: 该研究揭示了训练策略与系统延迟之间的相互作用，为在资源受限的物联网环境中开发更具韧性的入侵检测方案提供了重要见解。
---

## 摘要
物联网（IoT）设备的扩张增加了网络的攻击面，因此需要稳健且具有自适应能力的入侵检测系统。基于机器学习的系统在提高检测性能方面被认为具有广阔的前景。联邦学习设置使我们能够以隐私保护的方式，利用从客户端收集的网络入侵数据来训练模型。然而，由于概念漂移（即随着攻击者开发新技术，数据模式发生演变），这些系统的有效性可能会随着时间的推移而下降。现实的检测模型应当是非平稳的，以便能够利用新的入侵数据进行持续更新，同时保持对旧数据的检测能力。由于物联网环境受资源限制，更新应消耗最少的计算资源。本研究对增量联邦学习在增强物联网网络中非平稳入侵检测系统（IDS）模型长期性能方面的表现进行了全面的性能分析。具体而言，我们在联邦学习框架下提出了 LSTM 模型，以评估利用基于数据和模型的措施来应对漂移条件下灾难性遗忘的增量学习方法。利用包含五大类各种攻击变体的 CICIoMT2024 数据集，我们进行了二分类和多分类，以对入侵检测任务进行细粒度分析。结果表明，累积增量学习和代表性学习在漂移下提供了最稳定的性能，而基于保留的方法在准确性和延迟之间取得了良好的权衡。本研究为动态物联网环境中训练策略性能与延迟之间的相互作用提供了新的见解，旨在为考虑物联网设备资源限制的更具韧性的 IDS 解决方案的开发提供参考。
