...
tldr: 针对联邦学习中的数据异构性挑战，本文提出了一种基于客户端本地训练数据统计信息的条件联邦学习方法。该方法通过在本地计算PCA统计量来调节全局模型，无需额外通信开销。在多种异构场景和数据集上的实验表明，该方法在处理复杂异构性和数据稀疏性方面表现优异，甚至在某些场景下超越了已知真实聚类信息的基准模型。
selection_source: fresh_fetch
motivation: 现有的联邦学习方法在处理多维异构性和数据稀疏性时效果不佳，且往往伴随高昂的通信或计算成本。
method: 提出将全局模型以客户端本地计算的PCA统计量为条件进行调节，实现零额外通信开销的个性化适配。
result: "在97种实验配置下，该方法在复杂异构场景中比Oracle基准提升了1-6%，并展现出独特的稀疏鲁棒性。"
conclusion: 利用本地数据统计量进行模型调节是解决联邦学习异构性问题的一种高效且稳健的方案。
---

## 摘要
数据异构性下的联邦学习（FL）仍然具有挑战性：现有方法要么忽略客户端差异（FedAvg），要么需要昂贵的聚类发现（IFCA），或者为每个客户端维护模型（Ditto）。当数据稀疏或异构性是多维时，这些方法的性能都会下降。我们提出将单个全局模型以每个客户端训练数据的本地计算 PCA 统计量为条件，且无需额外的通信开销。通过在涵盖四种异构类型（标签偏移、协变量偏移、概念偏移和组合异构）、四个数据集（MNIST、Fashion-MNIST、CIFAR-10、CIFAR-100）以及七种联邦学习基准方法的 97 种配置下进行评估，我们发现我们的方法在所有设置中都达到了 Oracle 基准（即已知真实聚类分配的基准）的水平，在连续统计量比离散聚类标识符更丰富的组合异构设置中，其性能甚至超过了 Oracle 基准 1-6%，并且在所有测试方法中具有独特的稀疏鲁棒性。
