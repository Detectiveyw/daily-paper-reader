...
score: 6.0
evidence: 联邦推荐系统隐私风险
tldr: 联邦推荐系统在保护隐私的同时面临数据异构和稀疏性挑战，导致难以学习到泛化性强的物品嵌入。本文提出FedRecGEL框架，从物品中心视角将联邦推荐重新建模为多任务学习问题。通过引入锐度感知最小化（SAM）优化技术，该方法有效提升了物品嵌入的泛化能力和训练稳定性。在四个数据集上的实验证明，FedRecGEL显著优于现有联邦推荐方法。
selection_source: fresh_fetch
motivation: 现有联邦推荐系统忽视了在数据异构和稀疏环境下学习稳定且泛化性强的物品嵌入的重要性。
method: 提出FedRecGEL框架，将推荐任务建模为以物品为中心的多任务学习，并利用锐度感知最小化（SAM）来优化泛化性能。
result: 在四个公开数据集上的实验结果表明，FedRecGEL在推荐性能上显著优于现有的联邦推荐基准模型。
conclusion: 通过结合多任务学习视角与SAM优化技术，可以有效解决联邦推荐中的嵌入泛化难题，提升系统整体性能。
---

## 摘要
联邦推荐系统通过将用户交互数据保留在本地并仅共享必要的模型参数，实现了协作式模型训练，从而降低了隐私风险。然而，现有方法忽视了一个关键问题，即在整个联邦推荐系统训练过程中，如何稳定地学习广义物品嵌入。物品嵌入在促进跨客户端的知识共享中起着核心作用。然而，在跨设备场景下，本地数据分布表现出显著的异构性和稀疏性，加剧了学习广义嵌入的难度。这些因素使得稳定学习广义物品嵌入对于有效的联邦推荐既不可或缺，又在本质上难以实现。为了填补这一空白，我们提出了一种新的联邦推荐框架，命名为面向广义嵌入学习的联邦推荐（FedRecGEL）。我们从以物品为中心的视角重新定义了联邦推荐问题，并将其建模为一个多任务学习问题，旨在整个训练过程中学习广义嵌入。基于理论分析，我们采用锐度感知最小化（sharpness-aware minimization）来解决泛化问题，从而稳定训练过程并提升推荐性能。在四个数据集上的广泛实验证明了 FedRecGEL 在显著提升联邦推荐性能方面的有效性。我们的代码可在 https://github.com/anonymifish/FedRecGEL 获取。
