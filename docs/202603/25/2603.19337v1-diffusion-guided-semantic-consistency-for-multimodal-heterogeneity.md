...
tags: ["query:ppml"]
score: 7.0
evidence: 联邦学习中本地训练的隐私保护引导
tldr: 针对联邦学习中非独立同分布（non-IID）及多模态异构性导致的性能下降问题，本文提出SemanticFL框架。该框架利用预训练扩散模型（Stable Diffusion）的丰富语义表示，通过VAE潜变量和U-Net层次特征构建共享潜空间，并结合跨模态对比学习机制，在保护隐私的同时引导本地训练，显著提升了异构数据下的模型感知能力和收敛稳定性。
selection_source: fresh_fetch
motivation: 传统联邦学习难以处理客户端间的语义差异，导致在多模态异构场景下感知性能不佳。
method: 提出SemanticFL框架，利用预训练扩散模型的多层语义表示引导本地训练，并通过跨模态对比学习实现语义一致性。
result: "在CIFAR和TinyImageNet等数据集上的实验表明，该方法比FedAvg准确率最高提升了5.49%，优于现有主流联邦学习算法。"
conclusion: 通过引入扩散模型的语义先验，SemanticFL有效解决了联邦学习中的数据异构挑战，为多模态感知任务提供了更稳健的表示学习方案。
---

## 摘要
联邦学习 (FL) 面临非独立同分布 (non-IID) 客户端数据的严峻挑战，这一问题降低了全局模型的性能，尤其是在多模态感知场景中。传统方法往往无法解决客户端之间潜在的语义差异，导致需要鲁棒感知的多媒体系统性能欠佳。为了克服这一问题，我们提出了 SemanticFL，这是一个利用预训练扩散模型的丰富语义表示为本地训练提供隐私保护引导的新型框架。我们的方法利用预训练 Stable Diffusion 模型的多层语义表示（包括 VAE 编码的潜变量和 U-Net 分层特征）来创建一个对齐异构客户端的共享潜空间，并通过一种将繁重计算卸载到服务器的高效客户端-服务器架构来实现。一种采用跨模态对比学习的统一一致性机制进一步稳定了收敛。我们在包括 CIFAR-10、CIFAR-100 和 TinyImageNet 在内的基准数据集上，针对多种异构场景进行了广泛实验。实验结果表明，SemanticFL 优于现有的联邦学习方法，相比 FedAvg 实现了高达 5.49% 的准确率提升，验证了其在感知任务中为异构和多模态数据学习鲁棒表示的有效性。
