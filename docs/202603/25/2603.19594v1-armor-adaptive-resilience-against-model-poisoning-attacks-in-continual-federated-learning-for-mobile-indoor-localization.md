...
tags: ["query:ppml"]
score: 7.0
evidence: 用于移动定位的隐私保护联邦学习
tldr: 针对室内定位中持续联邦学习（CFL）易受环境动态变化和模型投毒攻击影响的问题，本文提出ARMOR框架。该框架引入状态空间模型（SSM）学习全局模型权重的历史演化并预测下一状态，通过对比实际更新与预测值来识别并过滤异常更新。实验表明，ARMOR在真实环境下显著提升了定位精度和系统鲁棒性，有效解决了模型性能退化问题。
selection_source: fresh_fetch
motivation: 室内定位环境的动态演变和潜在的模型投毒攻击会导致持续联邦学习中的全局模型产生偏差，进而引发性能严重退化。
method: 利用状态空间模型（SSM）预测全局模型权重的演化轨迹，并以此作为基准来检测和拦截偏离预期的异常本地更新。
result: 在真实世界测试中，ARMOR相比现有先进框架将平均定位误差降低了8.0倍，最坏情况下的误差降低了4.97倍。
conclusion: ARMOR通过结合时序预测与自适应防御机制，显著增强了持续联邦学习在复杂室内定位任务中的抗攻击能力和环境适应性。
---
