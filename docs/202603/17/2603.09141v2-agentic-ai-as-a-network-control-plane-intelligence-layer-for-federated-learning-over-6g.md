...
result: 案例研究表明，该系统通过灵活调用工具和闭环优化，在多变的网络环境下实现了高效的联邦学习性能。
conclusion: Agentic AI为6G联邦学习提供了一个智能化的控制平面，有效实现了学习任务与网络资源的协同优化。
---

## 摘要
向用户定制化端侧学习的转变对无线系统提出了新要求：模型必须在多样化、分布式数据上进行训练，同时满足严格的延迟、带宽和可靠性约束。为应对这一挑战，我们提出将代理式人工智能（Agentic AI）作为管理 6G 网络联邦学习（FL）的控制层，它能将高层任务目标转化为感知网络状况的行动。我们的系统不仅将 FL 视为学习挑战，还将其视为学习与网络管理的结合任务。一组专注于检索、规划、编码和评估的专业代理利用监控工具和优化方法，处理客户端选择、激励机制构建、调度、资源分配、自适应本地训练和代码生成。闭环评估和记忆的使用使系统能够持续优化决策，并考虑到不断变化的信噪比、带宽条件和设备能力。最后，我们的案例研究证明了代理式人工智能系统利用工具实现高性能的有效性。
