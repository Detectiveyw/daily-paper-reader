...
score: 7.0
evidence: 异步联邦学习聚合
tldr: 针对异步联邦学习中由于客户端计算速度差异导致的梯度过期问题，本文在AsyncFedED基础上，深入探讨了衡量梯度过期程度的不同距离度量指标。通过将这些指标集成到聚合过程中，研究了其在异构客户端和非独立同分布数据环境下对模型收敛速度、性能及训练稳定性的影响，为实际部署提供了更稳健的异步联邦学习方案。
selection_source: fresh_fetch
motivation: 异步联邦学习中传统的欧几里得距离可能无法准确捕捉梯度过期对模型收敛和准确性的负面影响。
method: 探索并集成多种替代距离度量指标到自适应聚合过程中，以更精确地衡量和处理梯度过期。
result: 实验表明，特定的距离度量指标能显著提升异步联邦学习在复杂环境下的训练效率和模型鲁棒性。
conclusion: 选择合适的距离度量指标是优化异步联邦学习聚合策略、提升系统整体性能的关键。
---

## 摘要
在异步联邦学习（FL）中，客户端设备根据其计算速度在不同时间向中央服务器发送更新，通常使用全局模型的陈旧版本。这种陈旧性会降低全局模型的收敛速度和准确性。先前的研究（如 AsyncFedED）提出了一种使用欧几里得距离来衡量陈旧性的自适应聚合方法。在本文中，我们通过探索替代的距离度量来扩展这一方法，以更准确地捕捉梯度陈旧性的影响。我们将这些度量集成到聚合过程中，并评估它们在异构客户端和非独立同分布（non-IID）数据设置下对收敛速度、模型性能和训练稳定性的影响。我们的结果表明，某些度量能够实现更稳健、更高效的异步联邦学习训练，为实际部署提供了更坚实的基础。
