...
## 摘要
无线网络上的联邦学习（FL）从根本上受到不可靠通信链路的限制，特别是当上行信道遭受遮挡、衰落或弱视距（LoS）条件影响时。夹持天线系统（PASS）提供了一种新的物理层能力，可以沿介质波导动态重新定位辐射点，从而实现可控的视距连接并显著提高信道质量。本文开发了 FedPASS，这是一个由 PASS 辅助的低延迟无线联邦学习的新型框架。我们制定了一个多目标优化问题，共同最小化端到端轮次延迟和联邦学习最优性间隙的上界。由此产生的公式是一个混合整数非线性规划问题，受限于调度、发射功率、本地 CPU 频率和 PA 放置的实际约束。为了应对由此产生的计算挑战，我们开发了一种两层迭代算法：外层循环通过块坐标下降法更新调度、通信时间分配和功率控制；内层循环在间距约束下，使用基于 Gauss-Seidel 的坐标更新和网格搜索来优化 PA 位置。在 MNIST 和 CIFAR-10 上的数值结果表明，FedPASS 实现了与理想化联邦学习基准相当的准确度，同时与传统无线联邦学习相比，大幅降低了总训练延迟。
