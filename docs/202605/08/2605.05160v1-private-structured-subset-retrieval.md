...
tldr: "本文提出了私有结构化子集检索（PSSR）问题，研究在 N 个非共谋服务器中检索具有特定结构限制的 D 个消息。该问题推广了经典的 PIR 和多消息 PIR（MPIR）。作者针对平衡 {0,1}-线性方案，推导了检索率和子分组级别的逆界，并开发了通用的优化框架。研究表明，利用需求结构可以显著提高检索率并降低子分组开销，在特定场景下优于现有的 MPIR 方案。"
selection_source: fresh_fetch
motivation: 传统的私有信息检索（PIR）未考虑检索需求中存在的特定结构限制，导致在特定应用场景下效率不高。
method: "针对平衡 {0,1}-线性方案，推导了最大检索率和最小子分组级别的理论界限，并提出了一个基于优化的通用方案构建框架。"
result: 实验证明 PSSR 方案在许多结构化需求族中实现了比 MPIR 更高的检索率，并显著降低了所需的子分组级别。
conclusion: 通过充分利用检索需求的结构化特征，可以打破传统 MPIR 的性能限制，实现更高效且低开销的私有数据检索。
---
