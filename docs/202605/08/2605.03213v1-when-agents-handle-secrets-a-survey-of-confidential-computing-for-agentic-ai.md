...
tldr: 本综述探讨了智能体AI（Agentic AI）在处理敏感信息时面临的独特安全威胁，如凭据窃取和上下文泄露。文章提出利用机密计算（CC）和可信执行环境（TEE）提供硬件级的安全保障。通过对六种TEE平台的分类、构建智能体威胁模型、评估现有防御措施以及识别多跳证明等挑战，为构建生产级安全智能体系统提供了理论框架和研究方向。
...
result: 明确了从单次推理到复杂智能体设计的安全迁移路径，并指出了复合远程证明和大规模GPU-TEE性能等六大核心挑战。
conclusion: 尽管硬件信任原语已趋于成熟，但目前仍缺乏一个能将这些技术整合为生产级智能体AI安全基座的统一端到端框架。
---

## 摘要
智能体 AI 系统，特别是那些能够进行规划、调用工具、维护持久内存，并通过 MCP 和 A2A 等协议将任务委托给同级智能体的大语言模型（LLM）驱动的智能体，引入了与独立模型推理有本质区别的威胁面。智能体积累敏感上下文、持有凭据，并在没有任何一方能完全控制的流水线中运行，这使得提示词注入、上下文泄露、凭据窃取以及智能体间消息投毒成为可能。当前的防御措施完全在软件栈内运行，可能会被具有足够权限的对手（如受损的云运营商）静默绕过。机密计算（CC）提供了一种基于硬件根的替代方案：可信执行环境（TEE）将智能体代码和数据与特权系统软件隔离，而远程证明（remote attestation）则实现了跨分布式部署的可验证信任。本综述从四个部分综合了设计空间：(i) 涵盖六种 TEE 平台（Intel SGX、Intel TDX、AMD SEV-SNP、ARM TrustZone、ARM CCA 和 NVIDIA H100 CC）的统一分类法，涉及部署角色和性能权衡；(ii) 一个以智能体为中心的威胁模型，涵盖感知、规划、内存、行动和协调层，并映射到九个安全目标；(iii) 对基于机密计算的防御措施进行的比较研究，区分了可从单次调用推理迁移的发现与需要全新智能体设计的发现；(iv) 六个开放性挑战，包括多跳智能体链的复合证明以及大语言模型规模下的 GPU-TEE 性能。尽管几种硬件信任原语在针对性部署方面已趋于成熟，但目前尚未建立起广泛认可的端到端框架，能将它们整合为生产级智能体 AI 的连贯安全基座。
