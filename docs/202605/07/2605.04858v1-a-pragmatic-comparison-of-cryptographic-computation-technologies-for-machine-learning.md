...
tldr: 随着安全需求增加，安全计算技术在机器学习中愈发重要，但从业者常面临技术选择难题。本文对比分析了安全多方计算（SMPC）和全同态加密（FHE）两大主流范式，总结了其理论优劣及开源实现。通过对多种机器学习模型进行广泛基准测试，揭示了不同技术在回归分析、简单密集网络及复杂卷积神经网络（CNN）中的性能差异，为从业者提供了实用的技术选择指南。
selection_source: fresh_fetch
motivation: 旨在解决从业者在面对多种安全计算技术时难以选择最适合机器学习任务的技术方案的问题。
method: 对SMPC和FHE的理论框架、开源软件实现进行总结，并针对不同机器学习模型开展了广泛的性能基准测试。
result: FHE在回归任务和简单密集网络中表现更优，而SMPC在处理卷积神经网络等复杂模型时展现出更卓越的性能。
conclusion: 本研究为机器学习安全计算提供了技术中立的基准参考，有效指导从业者根据具体应用场景选择最优加密技术。
---

## 摘要
随着安全需求的增加，安全计算技术的重要性日益凸显，但这些技术往往令从业者感到无从下手。此外，许多方法仅关注单一技术，可能会忽略更优的替代方案。本研究旨在通过对两种高度相关的密码学方法及其软件实现进行对比分析，解决安全计算技术的选择问题，并特别关注机器学习领域。首先，我们对安全多方计算 (SMPC) 和全同态加密 (FHE) 这两种安全计算范式进行了理论总结和比较。我们概述了这些协议的优缺点，以及相关的开源软件实现。其次，我们展示了针对机器学习算子和模型所确定的主要软件框架的广泛基准测试结果。关于 FHE 的当前技术水平，我们观察到它在回归任务中的表现优于 SMPC。此外，在使用 GPU 或混合模型处理简单的稠密网络时，它可能更快。相反，SMPC 在卷积神经网络 (CNN) 等复杂模型中表现出更优越的性能。我们的研究结果将为机器学习安全计算技术中更多与技术无关的基准测试铺平道路，并为寻求采用这些技术的从业者提供指导。
