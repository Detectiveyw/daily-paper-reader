...
tldr: 针对大推理模型和多智能体系统在中心化审计中面临的鲁棒性差、不可扩展、不透明及隐私泄露等问题，本文提出TRUST去中心化框架。该框架通过分层有向无环图分解推理过程，利用DAAN协议进行因果归因，并结合多层级共识机制确保审计的正确性。实验表明，TRUST在提高准确率的同时显著降低了成本，并能有效抵御恶意攻击，为安全可信的AI部署提供了去中心化审计方案。
selection_source: fresh_fetch
motivation: 解决中心化AI服务在处理复杂推理任务时存在的单点故障、黑盒操作及隐私安全风险。
method: 提出由分层有向无环图、DAAN因果归因协议及多层级权益加权共识机制组成的去中心化审计框架。
result: "实验显示该框架达到72.4%的准确率，在节省60% Token的同时实现了70%的根因归因成功率。"
conclusion: TRUST框架通过去中心化手段实现了透明、鲁棒且高效的AI审计，为高风险领域的AI应用提供了安全保障。
---

## 摘要
高风险领域中的大推理模型（LRMs）和多智能体系统（MAS）需要可靠的验证，但中心化方法存在四个局限性：(1) 鲁棒性，单点故障易受攻击和偏见影响；(2) 可扩展性，推理复杂性造成瓶颈；(3) 不透明性，隐藏的审计侵蚀信任；以及 (4) 隐私性，暴露的推理轨迹存在模型被盗风险。我们引入了 TRUST（透明、鲁棒且统一的可信 AI 服务），这是一个具有三项创新的去中心化框架：(i) 分层有向无环图（HDAGs），将思维链（CoT）推理分解为五个抽象层级，以进行并行分布式审计；(ii) DAAN 协议，将多智能体交互投影到因果交互图（CIGs）中，以实现确定性的根因归因；(iii) 计算检查器、LLM 评估器和人类专家之间的多层共识机制，采用权益加权投票，保证在 30% 对抗性参与下仍能确保正确性。我们证明了安全-盈利定理（Safety-Profitability Theorem），确保诚实的审计者获利，而恶意行为者遭受损失。所有决策均记录在链上，而隐私设计的分段机制防止了专有逻辑的重构。在多个 LLM 和基准测试中，TRUST 达到了 72.4% 的准确率（比基准高出 4-18%），并能抵御 20% 的损坏。DAAN 实现了 70% 的根因归因（标准方法为 54-63%），并节省了 60% 的 token。人类研究验证了该设计（F1 = 0.89，Brier = 0.074）。该框架支持 (A1) 去中心化审计、(A2) 防篡改排行榜、(A3) 无须信任的数据标注以及 (A4) 受监管的自主智能体，开创了去中心化 AI 审计的先河，旨在实现具备推理能力的系统的安全、负责任部署。
