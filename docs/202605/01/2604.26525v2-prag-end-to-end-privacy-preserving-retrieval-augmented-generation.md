...
tldr: 针对检索增强生成（RAG）在云端部署时的隐私泄露风险，本文提出PRAG系统。该系统通过双模式架构（非交互式PRAG-I和交互式PRAG-II）实现了文档和查询的端到端加密。PRAG引入了运算误差估计（OEE）机制来抵消同态加密噪声，确保语义排序的准确性。实验证明，PRAG在保持高召回率和低延迟的同时，能有效抵御图重构攻击，为大规模安全RAG提供了可行方案。
selection_source: fresh_fetch
motivation: 现有的隐私保护RAG方案往往在检索质量上有所妥协，或仅提供部分加密，无法兼顾数据安全与系统性能。
method: 提出包含非交互式和交互式两种模式的双模式架构，并引入运算误差估计（OEE）机制以稳定同态加密下的排名。
result: "在大规模数据集上实现了72.45%-74.45%的召回率，具有实用的检索延迟，并能强力抵御图重构攻击。"
conclusion: PRAG证明了在保证端到端机密性的前提下，实现高性能、可扩展的隐私保护RAG系统是完全可行的。
---

## 摘要
检索增强生成 (RAG) 对于利用外部知识增强大语言模型 (LLM) 至关重要，但其对云环境的依赖使敏感数据面临隐私风险。现有的隐私保护方案通常由于注入噪声而牺牲检索质量，或者仅提供部分加密。我们提出了 PRAG，一种端到端隐私保护 RAG 系统，在不牺牲云托管 RAG 可扩展性的情况下，实现了文档和查询的端到端机密性。PRAG 采用双模式架构：非交互式 PRAG-I 利用同态友好型近似来实现低延迟检索，而交互式 PRAG-II 则利用客户端辅助来达到与非隐私 RAG 相当的准确度。为了确保稳健的语义排序，我们引入了操作误差估计 (OEE)，这是一种针对同态噪声稳定排序的机制。在大规模数据集上的实验表明，PRAG 在保持端到端机密性的同时，实现了具有竞争力的召回率 (72.45%-74.45%)、实用的检索延迟以及对图重构攻击的强大抵御能力。这项工作证实了在大规模环境下实现安全、高性能 RAG 的可行性。
